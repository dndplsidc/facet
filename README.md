...
extensions with `pi remove` when they are no longer declared. Each extension entry
must provide a `source`; optional `install_env` values are passed only to that
extension's `pi install` command, which is useful for internal npm registries.
Manually installed Pi extensions are not touched. Use `facet apply --force` to
reinstall unchanged managed Pi extensions.
...
Skills without an explicit `agents` list default to `claude-code`, `cursor`, `codex`, and `pi`; the skills CLI name for Pi is `pi`. Installs are global (`-g`). Omitting `skills` requests `--skill "*"` with explicit agent scoping. Sources include GitHub repos, private SSH URLs, and local paths.

When a previously Facet-managed skill is removed entirely, facet removes that name across all CLI-supported agents without `-a` filters. The CLI cleans both `~/.agents/skills/<name>` and its global lock entry, including when Cline or another shared consumer is detected. If Codex or Cursor still needs the skill, shared storage remains and removal is scoped to dropped agents. An agent reduction leaving only Claude Code/Pi triggers global removal followed by native copies; native-only installs use `--copy`. Codex/Cursor still use `.agents/skills` even with `--copy`.

facet uses its existing `.state.json` to identify managed skills and leaves unrelated skill names alone. The skills CLI owns lock updates at `~/.agents/.skill-lock.json` (or `$XDG_STATE_HOME/skills/.skill-lock.json`). facet verifies global cleanup against both the lock and CLI inventory, retaining failed cleanup in existing state for retry. Post-install verification requires both lock metadata and actual installation, so stale lock entries do not become managed skills. Named installs retain their requested names with a warning if verification is unreadable. Remnants already missing from Facet state require explicit cleanup.

MCP servers are registered for Claude Code at user scope (`claude mcp add --scope user`); Cursor and Codex MCPs use their agent's config file. Unscoped MCPs do not target `pi`. See `facet docs ai` for details.
